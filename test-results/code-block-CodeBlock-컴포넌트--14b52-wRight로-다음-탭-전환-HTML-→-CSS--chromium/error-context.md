...
- Name: code-block.spec.js >> CodeBlock 컴포넌트 >> 키보드: ArrowRight로 다음 탭 전환 (HTML → CSS)
- Location: tests\code-block.spec.js:142:3
...
Call log:
  - waiting for locator('.code-block') to be visible
...
> 25  |   // ─────────────────────────────────────────────
  ...
  120 |   // 4. 클립보드 복사
  121 |   // ─────────────────────────────────────────────
  122 |   test('복사: Copy 클릭 시 Copied! 피드백이 2초간 표시되어야 한다', async ({ page }) => {
  123 |     const copyBtn = page.locator('.code-block__copy');
  124 |     await copyBtn.click();
  125 | 
```